...
description: Generate or edit images through 88api.ai with gpt-image-2.5-flare or gpt-image-2.5-sunburst over the OpenAI Images API. Use for text-to-image, reference-image editing, multi-reference composition, high-resolution requests, SSE previews, concurrent batches with one auto-group Key, repeated generation, workflow batch editing, or troubleshooting Codex Auto-mode sandbox and network-approval failures before an 88API request starts.
---
...
1. If the user explicitly requests the high-precision option, pass `--model gpt-image-2.5-sunburst` for that invocation.
2. If the user explicitly names `gpt-image-2.5-flare` or `gpt-image-2.5-sunburst`, pass that value with `--model`.
3. Otherwise omit `--model`; the CLI uses the saved model, then the factory default `gpt-image-2.5-flare`.
...
- `gpt-image-2.5-flare`: 2K matrix, default and fastest (up to ~50% lower latency than Image-2).
- `gpt-image-2.5-sunburst`: 4K matrix, precision-optimized for tighter editing control.
...
High-precision Sunburst model:

```powershell
node "<PLUGIN_ROOT>/scripts/generate.mjs" --model gpt-image-2.5-sunburst --prompt "<prompt>" --aspect 16:9
```
...
```powershell
node "<PLUGIN_ROOT>/scripts/generate.mjs" --edit --model gpt-image-2.5-sunburst --image "<person.png>" --image "<product.png>" --prompt "<composition instruction>" --aspect 16:9
```
...
```powershell
node "<PLUGIN_ROOT>/scripts/generate.mjs" --model gpt-image-2.5-sunburst --prompt "<prompt>" --aspect 16:9 --preview --dry-run
node "<PLUGIN_ROOT>/scripts/generate.mjs" --edit --model gpt-image-2.5-sunburst --image "<reference.png>" --prompt "<instruction>" --aspect 16:9 --dry-run
```
...
- `gpt-image-2.5-sunburst` 4:3 → `3264x2448`.
- `gpt-image-2.5-sunburst` 3:4 → `2448x3264`.
