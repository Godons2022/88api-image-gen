...
const MODEL_INFO = [
  {
    ...
    default: true,
    resolution: "2K",
    profile: "默认 2K，速度最优（对比 Image-2 延迟降约 50%）",
    recommendedFor: ["日常文生图", "常规参考图编辑", "高量批量生成"],
  },
  {
    id: "gpt-image-2.5-sunburst",
    default: false,
    resolution: "4K",
    profile: "4K 高分辨率 + 精细编辑控制",
    recommendedFor: ["精修创意图", "成片级宣发素材", "多轮精确编辑"],
  },
];
...
async function runWorkflowSelfTest() {
  ...
  const ok = firstQueue.length === 2
    ...
    && manifest.metadata?.model === "gpt-image-2.5-sunburst"
...
async function runImagesApiSelfTest() {
  ...
  const logSummary = imageApiLogSummary({ data: [{ b64_json: pngB64, url: "https://signed.example/image.png" }] }, "gpt-image-2.5-sunburst");
...
async function runImageStreamSelfTest() {
  ...
  const pngB64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==";
  const testModel = "gpt-image-2.5-sunburst";
...
async function runModelConfigSelfTest() {
  ...
  migrated.model = "gpt-image-2.5-sunburst";
  ...
  const ok = MODEL_INFO.length === 2
    ...
    && persisted.model === "gpt-image-2.5-sunburst"
    ...
    && effectiveModel({ model: "gpt-image-2.5-sunburst" }, persisted) === "gpt-image-2.5-sunburst"
    && effectiveModel({}, persisted) === "gpt-image-2.5-sunburst"
    && effectiveModel({}, {}) === DEFAULT_MODEL
    && resolveSize("gpt-image-2.5-flare", "16:9") === "2048x1152"
    && resolveSize("gpt-image-2.5-sunburst", "16:9") === "3840x2160"
    && resolveSize("gpt-image-2.5-sunburst", "4:3") === "3264x2448"
    && resolveSize("gpt-image-2.5-sunburst", "21:9") === null
    && invalidRejected
    && removedModelRejected
    && dryRun.paidApiCalled === false
    && dryRun.request.body.model === "gpt-image-2.5-sunburst"
...
DEFAULTS
  ...
  resolution: gpt-image-2.5-flare=2K; gpt-image-2.5-sunburst=4K
